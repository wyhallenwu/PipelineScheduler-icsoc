...
Controller::Controller(int argc, char **argv)
{
    ...
    client_profiles_jf = ClientProfilesJF();
    model_profiles_jf = ModelProfilesJF();
}
...
void Controller::Scheduling()
{
    while (running)
    {
        ...
        auto server_device = this->model_profiles_jf.infos.begin()->second[0].model->device;
...
void Controller::AddTask(const TaskDescription::TaskStruct &t)
{
    ...
    std::vector<std::pair<std::string, std::string>> possibleDevicePairList = {{t.device, "server"}};
    ...
    for (auto &model : ctrl_unscheduledPipelines.back().pipelineModels)
    {
        ...
        if (containerName.find("datasource") != std::string::npos || containerName.find("sink") != std::string::npos)
        {
            ...
            if (containerName.find("datasource"))
            {
                ...
                this->client_profiles_jf.add(containerName, t.slo, model->arrivalProfiles.arrivalRates,
                                             model, t.name, t.source, possibleNetworkEntryPairs);
            }
            // -------------------
            continue;
        }
