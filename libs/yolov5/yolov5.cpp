...
YoloV5Agent::YoloV5Agent(const std::string &name, uint16_t own_port, std::vector<BaseMicroserviceConfigs> &msvc_configs,
                         const TRTConfigs &yoloConfigs, Microservice *receiver)
        : ContainerAgent(name, own_port) {
    msvcs.push_back(receiver);
...
int main(int argc, char **argv) {
    ...
    ContainerAgent *agent = new YoloV5Agent(name, absl::GetFlag(FLAGS_port), msvc_configs, yoloConfigs,
                                            new Receiver(msvc_configs[0], CommMethod::localGPU));
    while (agent->running()) {
